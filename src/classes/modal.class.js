...
class Modal {
  constructor(options, onclose) {
    ...
    // Predefined grid positions to guarantee full screen coverage
    let positionStyle = "";
    if (options.centerPosition) {
      // Center position for final dialog
      positionStyle = `position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); z-index: 9999 !important;`;
    } else if (options.randomPosition) {
      // Initialize positions array on first use - covers entire screen
      if (!window._modalPositions) {
        window._modalPositions = [];
        // Create dense grid: 8 columns x 6 rows = 48 positions
        for (let row = 0; row < 6; row++) {
          for (let col = 0; col < 8; col++) {
            window._modalPositions.push({
              x: 6 + col * 12.5, // 6%, 18.5%, 31%, 43.5%, 56%, 68.5%, 81%, 93.5%
              y: 8 + row * 16, // 8%, 24%, 40%, 56%, 72%, 88%
            });
          }
        }
        // Shuffle the positions for randomness
        for (let i = window._modalPositions.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [window._modalPositions[i], window._modalPositions[j]] = [
            window._modalPositions[j],
            window._modalPositions[i],
          ];
        }
      ...
      const pos =
        window._modalPositions[
          window._modalPosIndex % window._modalPositions.length
        ];
      window._modalPosIndex++;

      positionStyle = `position: fixed; left: ${pos.x}%; top: ${pos.y}%; transform: translate(-50%, -50%);`;
    }

    let DOMstring = `<div id="modal_${this.id}" class="${this.classes}" style="z-index:${zindex + Object.keys(window.modals).length}; ${positionStyle}" augmented-ui="${augs.join(" ")} exe">
            <h1>${this.title}</h1>
