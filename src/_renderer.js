...
window.onerror = (msg, path, line, col, error) => {
  document.getElementById("boot_screen").innerHTML +=
    `${error} :  ${msg}<br/>==> at ${path}  ${line}:${col}`;
};
...
async function initUI() {
  ...
  document.body.innerHTML += `
    ...
    <section id="bottom-panel" style="opacity: 0;">
      <section id="satellite-panel">
        ...
        <div class="satellite-content">
          <div class="satellite-stats">
            <div class="stat-card">
              <div class="stat-label">ORBITAL STATUS</div>
              <div class="stat-value healthy">NOMINAL</div>
            </div>
            <div class="stat-card">
              <div class="stat-label">ALTITUDE</div>
              <div class="stat-value" id="sat-altitude">408.2 km</div>
            </div>
            <div class="stat-card">
              <div class="stat-label">VELOCITY</div>
              <div class="stat-value" id="sat-velocity">7.66 km/s</div>
            </div>
            <div class="stat-card">
              <div class="stat-label">SIGNAL</div>
              <div class="stat-value" id="sat-signal">98.7%</div>
            </div>
          </div>
          <div class="satellite-charts">
            <div class="chart-container">
              <div class="chart-title">ORBIT TRAJECTORY</div>
            ...
            <div class="chart-container">
              <div class="chart-title">POWER SYSTEMS</div>
              <div class="power-gauge">
                <svg viewBox="0 0 100 50">
                  <path d="M10,50 A40,40 0 0,1 90,50" fill="none" stroke="currentColor" stroke-width="8" opacity="0.15"/>
                  <path id="power-arc" d="M10,50 A40,40 0 0,1 90,50" fill="none" stroke="currentColor" stroke-width="8" stroke-dasharray="126" stroke-dashoffset="12"/>
                </svg>
                <div class="power-value">92%</div>
              </div>
            </div>
          </div>
          <button id="satellite-see-more" class="see-more-btn">[ SEE MORE DETAILS ]</button>
        </div>
      </section>
    </section>`;

  // Satellite panel animations and interactions
  function initSatellitePanel() {
    // Animate satellite dot orbit
    const satelliteDot = document.querySelector(".satellite-dot");
    if (satelliteDot) {
      let angle = 0;
      setInterval(() => {
        angle = (angle + 2) % 360;
        ...
        satelliteDot.style.left = x + "%";
        satelliteDot.style.top = y + "%";
      }, 50);
    }
    ...
    // Update altitude and velocity with slight variations
    setInterval(() => {
      const altEl = document.getElementById("sat-altitude");
      const velEl = document.getElementById("sat-velocity");
      const sigEl = document.getElementById("sat-signal");
      if (altEl)
        altEl.textContent = (408 + Math.random() * 0.5).toFixed(1) + " km";
      if (velEl)
        velEl.textContent = (7.66 + Math.random() * 0.02).toFixed(2) + " km/s";
      if (sigEl)
        sigEl.textContent = (97 + Math.random() * 2.5).toFixed(1) + "%";
    }, 2000);

    // See More button - PANIC MODE
    ...
    if (seeMoreBtn) {
      seeMoreBtn.addEventListener("click", () => {
        ...
        setTimeout(() => {
          // HIJACK SPAM EFFECT - Flood screen with dialogs
          const baseMessages = [
            { title: `SECURITY BREACH`, msg: `UNAUTHORIZED ACCESS DETECTED` },
            { title: `SATELLITE HIJACKED`, msg: `CONTROL SYSTEMS COMPROMISED` },
            { title: `CRITICAL ALERT`, msg: `THRUSTER CONTROL LOST` },
            { title: `ORBIT FAILURE`, msg: `TRAJECTORY UNSTABLE` },
            { title: `HOSTILE TAKEOVER`, msg: `FOREIGN SIGNAL DETECTED` },
            { title: `COLLISION WARNING`, msg: `EARTH IMPACT IMMINENT` },
            { title: `SYSTEM BREACH`, msg: `FIREWALL BYPASSED` },
            { title: `ALTITUDE DROP`, msg: `ORBITAL DECAY DETECTED` },
            { title: `COMMS HIJACKED`, msg: `MISSION CONTROL OFFLINE` },
            { title: `WARNING`, msg: `MULTIPLE SYSTEM FAILURES` },
            { title: `EMERGENCY`, msg: `BACKUP SYSTEMS FAILING` },
            { title: `INTRUSION`, msg: `HOSTILE CODE INJECTED` },
            { title: `LOCKOUT`, msg: `ADMIN ACCESS REVOKED` },
            { title: `CRITICAL`, msg: `PROPULSION OFFLINE` },
            { title: `SIGNAL LOST`, msg: `NO UPLINK RESPONSE` },
            { title: `HULL BREACH`, msg: `STRUCTURAL DAMAGE` },
            { title: `GPS FAILURE`, msg: `POSITION UNKNOWN` },
            { title: `TELEMETRY LOST`, msg: `ALL SENSORS OFFLINE` },
            { title: `SATELLITE LOST`, msg: `REMOTE ACCESS DENIED` },
            { title: `THERMAL ALERT`, msg: `OVERHEATING DETECTED` },
            { title: `GUIDANCE FAILURE`, msg: `NAVIGATION OFFLINE` },
            { title: `NETWORK BREACH`, msg: `GROUND STATIONS DOWN` },
            { title: `ABORT FAILED`, msg: `RECOVERY IMPOSSIBLE` },
            { title: `TOTAL FAILURE`, msg: `ALL SYSTEMS COMPROMISED` },
            { title: `HOSTILE ACCESS`, msg: `ATTACKER IN CONTROL` },
            { title: `ORBIT DECAY`, msg: `ALTITUDE CRITICAL` },
            { title: `ENCRYPTION BROKEN`, msg: `SECURITY COMPROMISED` },
            { title: `BACKDOOR FOUND`, msg: `UNAUTHORIZED ACCESS` },
            { title: `REENTRY WARNING`, msg: `ATMOSPHERE CONTACT` },
            { title: `SABOTAGE`, msg: `SYSTEMS DESTROYED` },
          ];

          // Reset position tracker for fresh coverage
          ...
          // Spawn multiple dialogs with slight delays for dramatic effect
          hijackMessages.forEach((msg, index) => {
            setTimeout(() => {
              new Modal({
                type: "custom",
                title: msg.title,
              ...
              // Play warning sound for each dialog
              if (window.audioManager && window.audioManager.denied) {
          ...
          setTimeout(() => {
            new Modal({
              ...
              html: `
                <div style="text-align: center; font-family: monospace; padding: 30px; background: rgba(0,0,0,0.8);">
                  <h1 style="color: #ff0000; font-size: 2.5em; margin-bottom: 20px; text-shadow: 0 0 20px #ff0000;">
                    SATELLITE OUT OF ORBIT
                  </h1>
                  <div style="background: linear-gradient(180deg, #300 0%, #100 100%); padding: 25px; border: 4px solid #ff0000; margin: 20px 0;">
                    <p style="color: #ff6600; font-size: 1.8em; margin-bottom: 15px;">
                      EDEX-SAT-07 HAS LOST ORBITAL STABILITY
                    </p>
                    <p style="color: #ff3333; font-size: 1.5em; margin-bottom: 20px;">
                      TRAJECTORY: <span style="color: #ff0000; font-weight: bold;">EARTH COLLISION COURSE</span>
                    </p>
                    ...
                    <p style="color: #ff6600; font-size: 1.1em; margin-top: 15px;">
                      IMPACT ZONE: FOE at UOR
                    </p>
                  </div>
                  <p style="color: #ff0000; font-size: 1.4em; font-weight: bold; margin-top: 20px;">
            ...
            if (countdownEl) {
              ...
              const countdownInterval = setInterval(() => {
                ...
                if (totalSeconds <= 0) {
                  ...
                  countdownEl.innerHTML = "IMPACT IMMINENT";
                  return;
                }
  ...
  // Show bottom panel with satellite after welcome screen
  document.getElementById("bottom-panel").setAttribute("style", "opacity: 1;");

  // Initialize satellite panel animations
  initSatellitePanel();
  ...
  new Modal(
    {
      ...
      buttons: [
        ...
        {
          label: "Restart eDEX",
          action: "remote.app.relaunch();remote.app.quit();",
        },
      ],
    },
    () => {
